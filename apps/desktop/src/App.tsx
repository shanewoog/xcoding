...
function eventActivity(event: SessionEvent): Activity | null {
  if (event.type === "tool_start") {
    return {
      ...
      kind: "tool",
      label: event.summary,
      detail: JSON.stringify(event.tool_call.arguments),
      state: "running",
    };
  }
  ...
  if (event.type === "error") {
    return { id: `error-${Date.now()}`, kind: "error", label: "Agent error", detail: event.message, state: "failed" };
  }
  return null;
}
  ...
  useEffect(() => {
    ...
    void listen<SessionEvent>("session-event", (event) => {
      ...
      if (payload.type === "text_delta") {
        setStreamedText((current) => current + payload.delta);
      }
      if (payload.type === "message_completed") {
        setCompletedText(payload.message.content);
        setStreamedText("");
      }
      ...
      if (nextActivity) {
        setActivity((current) => {
          ...
          return current.map((item) => (item.id === nextActivity.id ? nextActivity : item));
        });
      }
    }).then((stop) => {
      unlisten = stop;
    ...
    <main className="workbench">
      <aside className="sessions-panel" aria-label="Sessions">
        <div className="brand-row">
          ...
          <button type="button" className="quiet-button" onClick={() => void refreshSessions()} aria-label="Refresh sessions">
            Refresh
          </button>
        </div>
      ...
      <section className="chat-panel" aria-label="Coding conversation">
        <header className="chat-header">
          <div>
            <p className="eyebrow">Cloud model</p>
            <h2>{activeSession ? sessionTitle(activeSession) : "New coding task"}</h2>
          </div>
          <span className="mode-badge">Ask mode</span>
        </header>
        ...
        <form className="composer" onSubmit={submit}>
          ...
          <div className="composer-footer">
            <span>{workspaceRoot.trim() ? workspaceRoot : "Choose a workspace path"}</span>
      ...
      <aside className="trace-panel" aria-label="Agent trace">
        <section>
          <p className="panel-title">Plan</p>
        ...
        <section>
          ...
          <div className="activity-list">
            {activity.length === 0 ? <p className="empty-state">Read-only tool activity will be recorded here.</p> : null}
            {activity.map((item) => (
