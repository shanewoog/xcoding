...
供应商配置带有 `trust_level`：`local` 用于本地或私有化服务，`official` 用于确认过的官方直连，`relay` 用于中转站及未知服务。历史配置和未声明该字段的供应商默认按 `relay` 处理，不会因为配置升级而自动获得信任；供应商兜底也不会跨越信任级别。

在 `relay` 供应商上：

- 检测到私钥、常见云凭据、Bearer/JWT、`.env` 或类似高置信度敏感内容时，请求会被阻断；无法可靠解析消息时也按敏感内容处理。
- 图片不会发送给中转站的视觉委托供应商。
- `apply_patch`、`run_command` 和 MCP 工具即使原本可自动执行，也会降级为需要用户确认。
- 会校验供应商返回的模型标识；声明模型与请求模型不一致时，本轮调用失败，不接受中转站静默换模型。

这不是对中转站“可信”的证明。涉密内容应优先使用本地或确认过的官方直连，并在网络、供应商日志和访问权限层面另行落实审计与留存策略。

## 命令策略

`run_command` 由硬拒绝名单、白名单、可选工作区名单与风险标注共同约束：

- **硬拒绝**：明显危险的系统命令（例如 `format`、`shutdown`、递归删除根路径、注册表机器范围修改、`git clean`、`git push --mirror`）。硬拒绝命令不会进入审批队列。
