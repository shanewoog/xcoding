...
在工作区文本文件中搜索字符串。可选：`case_insensitive`、简单 `glob`（`*` / `?`）、`context_lines`（0–3）。源码路径会优先排序；常见构建目录会被忽略。
...
只读。加载工作区 skill 全文：`.xcoding/skills/<name>/SKILL.md`。
...
skill 名称即目录名：`1-64` 个字符，字符集 `[A-Za-z0-9._-]`，且以字母或数字开头。可选 YAML frontmatter 可写 `name`（须与目录名一致）和 `description`；frontmatter 之后的正文为 skill 内容。

### `mcp`

高风险 exec。调用已配置的 MCP 工具。模型侧使用命名空间名 `mcp__<server>__<tool>`；协议层规范为：
...
服务器在 `.xcoding/mcp.json` 的 `mcpServers` 中声明（stdio 的 `command`/`args`/`env`/`enabled`）。每次 MCP 调用都需要用户审批。结果示例：
...
`revision` 必填（commit-ish）。`path` 可选，工作区相对路径。
...
暂存工作区相对路径。始终归类为**高风险写入**（会修改 `.git`），因此 `ask` 与 `auto-edit` 都需要审批。
...
`paths` 必填，且必须是非空的工作区相对路径数组。绝对路径、`..` 以及包含 `.git` / `.xcoding` 段的路径会被拒绝。
...
使用提交信息创建 commit。始终归类为**高风险写入**。本版本不支持 amend / `--no-verify` / force。
...
`message` 必填（trim 后非空）。`allow_empty` 默认 false。
...
将分支推送到远端。始终归类为**高风险写入**（会更新远端引用）。`ask` 与 `auto-edit` 都需要审批。本版本**从不 force-push**。
...
- `remote` 可选，默认 `origin`。必须是单个远端名（不以 `-` 开头、无空白、不含 `:` / `..`）。
- `branch` 可选，默认当前分支。detached HEAD 时必须显式传入 branch。
- `set_upstream` 可选，默认 false（为 true 时等价于 `git push --set-upstream`）。
...
鉴权 / 网络失败会通过 git stderr 以工具错误形式返回（例如缺少凭据或 non-fast-forward 被拒绝）。


### `git_fetch`

从远端抓取更新。始终归类为**高风险写入**（更新本地 remote-tracking 引用）。`ask` 与 `auto-edit` 都需要审批。不 force、不 prune 危险选项。
...
- `remote` 可选，默认 `origin`。必须是单个远端名（不以 `-` 开头、无空白、不含 `:` / `..`）。
- `branch` 可选；省略时抓取该远端默认配置（通常为全部跟踪分支）。
...
从远端拉取并更新当前工作分支。始终归类为**高风险写入**。`ask` 与 `auto-edit` 都需要审批。默认 `--ff-only`；`ff_only: false` 时仅允许 `--no-rebase` 合并拉取。永不 force / rebase。
