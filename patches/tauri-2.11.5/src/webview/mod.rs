...
impl<R: Runtime> Webview<R> {
  ...
  /// Export webview content as PDF using the native WebView2 `PrintToPdf` API.
  /// The callback fires with `true` on success.
  ///
  /// ## Platform-specific
  ///
  /// - **Windows**: Uses the WebView2 `PrintToPdf` COM interface.
  /// - **Other platforms**: The callback receives `false`.
  pub fn print_to_pdf(
    &self,
