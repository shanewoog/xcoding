...
pub enum WebviewMessage {
  ...
  Reload,
  Print,
  PrintToPdf(std::path::PathBuf, Box<dyn FnOnce(bool) + Send>),
  Close,
  Show,
  Hide,
  SetPosition(Position),
...
impl<T: UserEvent> WebviewDispatch<T> for WryWebviewDispatcher<T> {
  ...
  fn print_to_pdf(&self, path: std::path::PathBuf, cb: Box<dyn FnOnce(bool) + Send>) -> Result<()> {
    send_user_message(
      &self.context,
      Message::Webview(
        *self.window_id.lock().unwrap(),
        self.webview_id,
        WebviewMessage::PrintToPdf(path, cb),
      ),
    )
  }
    ...
    Message::Webview(window_id, webview_id, webview_message) => {
      ...
      if let Some((Some(window), Some(webview))) = webview_handle {
        match webview_message {
          ...
          WebviewMessage::PrintToPdf(path, cb) => {
            let _ = webview.print_to_pdf(&path, cb);
          }
          WebviewMessage::Close => {
