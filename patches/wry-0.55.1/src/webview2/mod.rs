...
impl InnerWebView {
  ...
  /// Export the webview content as PDF to the given path using the native
  /// WebView2 `PrintToPdf` API. The callback is invoked with `true` on success.
  pub fn print_to_pdf(
    &self,
    path: &std::path::Path,
    ...
    unsafe {
      let webview7: ICoreWebView2_7 = self.webview.cast()?;
      let env6: ICoreWebView2Environment6 = self.env.cast()?;
      ...
      let handler = PrintToPdfCompletedHandler::create(Box::new(
        move |result: std::result::Result<(), windows::core::Error>, is_success: bool| {
          cb(result.is_ok() && is_success);
          Ok(())
        },
      ));
