...
async function main() {
  ...
  await assertCompactionSummaryCarriesTheDescription();
...
async function assertDelegateReplacesImagesAndCachesTheDescription() {
  ...
  try {
    ...
    assert.equal(applied.length, 1, "an applied description should be observable");
    assert.equal(applied[0].image_count, 1);
    assert.equal(
      applied[0].historical_chars,
      0,
      "the newest attachment must not be charged against the historical budget",
    );
    assert.equal(applied[0].truncated, false);
...
async function assertHistoricalAttachmentIsLabelledAsSuch() {
  ...
  try {
    ...
    // Restart drops the in-memory cache and clearing the table drops the
    // persisted one, so the second turn has to describe the earlier attachment
    // again. The cache key is derived from the image bytes alone, so changing
    // the delegate model would no longer force a fresh description.
    await context.restart({ clearStoredDescriptions: true });
    ...
    assert.ok(
      applied[0].historical_chars > 0,
      "an earlier attachment must be charged against the historical budget",
    );
  } finally {
    await context.close();
    await vision.close();
...
/// Compaction is where an image is lost for good: once the message is replaced
/// by a summary, a session model without vision can never recover it. The
/// summary prompt must therefore carry the cached description instead of an
/// opaque `[1 image attachment(s)]` placeholder.
async function assertCompactionSummaryCarriesTheDescription() {
  const padding = "x".repeat(4_000);
  const vision = await startMockProvider({ text: VISION_DESCRIPTION });
  const session = await startMockProvider({ text: `Acknowledged. ${padding}` });
  const context = await startIsolatedServer({
    slug: "vision-compaction",
    sessionBaseUrl: session.baseUrl,
    visionBaseUrl: vision.baseUrl,
    // A small window plus the lowest allowed threshold makes compaction fire on
    // the fifth turn instead of depending on the real model window.
    configOverrides: {
  ...
  try {
    const first = await context.rpc.request("session.chat", {
      workspace_root: fixtureRoot,
      message: `Why does this screen fail? ${padding}`,
      model: "fixture-model",
      images: [{ mime_type: "image/png", data_base64: PNG_BASE64, name: "screen.png" }],
    });
    assert.equal(first.session.status, "done");
    ...
    // Four more turns push the history past the recent-message floor, so the
    // oldest message (the one carrying the attachment) gets compacted.
    ...
    const compaction = session.requests.find((request) =>
      /You compact earlier history/.test(contentText(request.messages[0])),
    );
    assert.ok(compaction, "the fifth turn should have compacted the oldest message");
    const body = contentText(compaction.messages.at(-1));
    assert.match(
      body,
      /\[1 image attachment\(s\), described earlier as:\]/,
      "the summary prompt should announce the described attachment",
    );
    assert.match(body, /INVALID_API_KEY/, "the summary prompt should carry the description text");
    assert.ok(
      !body.includes(PNG_BASE64),
      "the summary prompt must not carry the base64 payload",
    );
    assert.equal(
      vision.requests.length,
      1,
      "building a summary must never call the delegate provider",
    );
  } finally {
    await context.close();
    await vision.close();
...
async function startIsolatedServer({ slug, sessionBaseUrl, visionBaseUrl, configOverrides }) {
      ...
      `${JSON.stringify(
        {
          ...
          providers: [
            { id: "default", name: "openai", base_url: sessionBaseUrl, api_key: "e2e-session-key", trust_level: "official" },
            { id: "vision", name: "vision", base_url: visionBaseUrl, api_key: "e2e-vision-key", trust_level: "official" },
          ],
  ...
  return {
    ...
    restart: async ({ visionModel, clearStoredDescriptions } = {}) => {
      await rpc.close();
      if (visionModel) {
        await writeConfig(visionModel);
      }
      // Only safe while no server holds the database, which is why this runs
      // between the two processes.
      if (clearStoredDescriptions) {
        const database = new DatabaseSync(databasePath);
        try {
          database.exec("DELETE FROM vision_descriptions");
        } finally {
          database.close();
        }
      }
