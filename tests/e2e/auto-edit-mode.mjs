...
async function main() {
  const workspace = await mkdtemp(resolve(tmpdir(), "xcoding-auto-edit-"));
  const databaseDirectory = await mkdtemp(resolve(tmpdir(), "xcoding-auto-edit-db-"));
  const homeDirectory = await mkdtemp(resolve(tmpdir(), "xcoding-auto-edit-home-"));
  ...
  try {
    ...
    const auto = await rpc.request("session.chat", {
      workspace_root: workspace,
      message: "Add auto marker.",
      model: "fixture-model",
      mode: "auto-edit",
    });
    assert.equal(auto.session.mode, "auto-edit");
    ...
    assert.ok(autoStart);
    assert.match(String(autoStart.summary), /Auto-applying apply_patch/i);

    // Task 6b: auto-edit still gates commands after auto-applied patch.
    mock.scenario = "auto-patch-then-command";
    const gated = await rpc.request("session.chat", {
      workspace_root: workspace,
      message: "Patch then command.",
      model: "fixture-model",
      mode: "auto-edit",
    });
    assert.equal(gated.session.status, "need_user");
    const commandApproval = eventFor(rpc, gated.session.id, "approval_requested");
    assert.equal(commandApproval.action.tool_call.name, "run_command");
    assert.ok(
      eventsFor(rpc, gated.session.id).some(
        (event) =>
          event.type === "tool_end" &&
    ...
    assert.equal(await readFile(resolve(workspace, "gated.txt"), "utf8"), "gated\n");
    const commandStart = eventsFor(rpc, gated.session.id).find(
      (event) => event.type === "tool_start" && event.tool_call?.name === "run_command",
    );
    assert.ok(commandStart);
    assert.match(String(commandStart.summary), /Awaiting approval for run_command/i);
    const afterReject = await rpc.request("session.resolve", {
      session_id: gated.session.id,
      action_id: commandApproval.action.id,
      approved: false,
    });
    assert.equal(afterReject.session.status, "done");
    assert.match(afterReject.message?.content ?? "", /command remains gated/i);

    // High-risk path still requires approval under auto-edit.
    mock.scenario = "auto-high-risk-path";
    ...
    assert.equal(highRisk.session.status, "need_user");
    const riskApproval = eventFor(rpc, highRisk.session.id, "approval_requested");
    assert.equal(riskApproval.action.tool_call.name, "apply_patch");
    assert.equal(riskApproval.action.tool_call.arguments.path, ".xcoding/secret.txt");
    await assert.rejects(readFile(resolve(workspace, ".xcoding/secret.txt"), "utf8"));
    await rpc.request("session.resolve", {
      session_id: highRisk.session.id,
      action_id: riskApproval.action.id,
      approved: false,
    });
    await assert.rejects(readFile(resolve(workspace, ".xcoding/secret.txt"), "utf8"));

    console.log("Auto-edit mode policy checks passed.");
  } finally {
    await rpc.close();
    await mock.close();
...
async function startMockProvider() {
  const state = { scenario: "ask-patch" };
  const server = createServer(async (request, response) => {
    ...
    if (state.scenario === "ask-patch" || state.scenario === "auto-patch") {
      if (hasToolResult) {
        writeText(response, "The requested marker is ready.");
      } else {
        const path = state.scenario === "auto-patch" ? "auto.txt" : "ask.txt";
        const text = state.scenario === "auto-patch" ? "automatic\n" : "ask-me\n";
        writeTool(response, "call_patch", "apply_patch", { path, old_text: "", new_text: text });
      }
      response.end("data: [DONE]\n\n");
      return;
    }

    if (state.scenario === "auto-patch-then-command") {
      if (hasRejection || toolMessages.length >= 2) {
        writeText(response, "The command remains gated behind approval.");
      } else if (toolMessages.length === 1) {
        const executable = process.platform === "win32" ? "cmd" : "true";
        const args = process.platform === "win32" ? ["/c", "echo", "gated"] : [];
        writeTool(response, "call_cmd", "run_command", { executable, args });
      } else {
        writeTool(response, "call_patch", "apply_patch", {
          path: "gated.txt",
          old_text: "",
          new_text: "gated\n",
        });
    ...
    if (state.scenario === "auto-high-risk-path") {
      if (hasToolResult) {
        writeText(response, "Protected path stayed gated.");
      } else {
